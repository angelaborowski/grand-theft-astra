...
export function mapDestination(
  ...
  if (isInsideMuseum(player.position))
    return {
      id: SCENE_IDS.square,
      name: "Exit to Red Square",
      position: { x: MUSEUM.originX, z: 0 },
      kind: "destination",
    };
...
export function mapMarkers(
  ...
  for (const entity of snapshot.entities) {
    ...
    if (sceneSpace(entity.position) !== sceneSpace(player.position)) continue;
