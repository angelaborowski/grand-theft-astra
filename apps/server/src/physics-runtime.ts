...
type Space = "district" | "guesthouse" | "museum";
type Body = {
...
export class PhysicsRuntime {
  ...
  constructor(world: WorldSnapshot, now: number) {
    ...
    this.spaces = {
      ...
      museum: new RAPIER.World({ x: 0, y: 0, z: 0 }),
    };
    ...
    for (const box of MUSEUM_COLLIDERS) {
      this.spaces.museum.createCollider(
        RAPIER.ColliderDesc.cuboid(box.halfExtents[0], box.halfExtents[1], box.halfExtents[2])
          ...
          .setRotation({
            x: Math.sin(box.rotationX / 2),
            y: 0,
            z: 0,
            w: Math.cos(box.rotationX / 2),
          }),
      );
    }
  ...
  control(
    ...
    if (spaceFor(player.position) === spaceFor(this.player(next, id)?.position ?? player.position))
  ...
  afterAction(previous: WorldSnapshot, next: WorldSnapshot, id: EntityId): void {
    ...
    if (
      spaceFor(before.position) === spaceFor(after.position) &&
      beforeVehicle === afterVehicle &&
      Math.hypot(before.position.x - after.position.x, before.position.z - after.position.z) < 0.01
    )
      return;
  ...
  validateAction(
    ...
    if (
      ...
      !player.grounded ||
      player.elevation - museumFloorHeight(player.position) > 0.2
    )
    ...
    if (
      ...
      action.type === "visit_museum" ||
      action.type === "launch_stunt"
    )
      return undefined;
    this.synchronize(world);
  ...
  private walk(player: Player, input: PlayerControl | undefined): Player {
    ...
    if (
      ...
      this.controls.delete(player.id);
      body.verticalSpeed = 0;
      body.speed = 0;
      return {
        ...player,
