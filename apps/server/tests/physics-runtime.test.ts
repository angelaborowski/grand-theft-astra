...
import { SCENE_IDS, GUESTHOUSE, MUSEUM } from "@gpta/core/scene";
...
test("walks the museum upper hall and stairs into the square using authoritative controls", async ({
  simulate,
}) => {
  const upper = await simulate({
    position: MUSEUM.spawn,
    elevation: 1.2,
    instructions: controls({ forward: 1, cameraYaw: Math.PI }, 1000),
  });
  expect(upper.player.position.z).toBeGreaterThan(MUSEUM.spawn.z);
  expect(upper.player.elevation).toBeCloseTo(1.2, 1);
  const outside = await simulate({
    position: MUSEUM.spawn,
    elevation: 1.2,
    instructions: controls({ forward: 1, cameraYaw: Math.PI }, 7500),
  });
  expect(outside.player.position.x).toBeCloseTo(MUSEUM.exit.x, 1);
  expect(outside.player.position.z).toBeGreaterThanOrEqual(MUSEUM.exit.z);
  expect(outside.player.position.z).toBeLessThan(-120);
  expect(outside.player.elevation).toBeCloseTo(0, 1);
});
