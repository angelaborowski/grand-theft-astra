...
function at(position: Position): WorldSnapshot {
  ...
  return {
    ...
    entities: world.entities.map((entity) =>
      entity.id === SCENE_IDS.player ? { ...entity, position, elevation: 0 } : entity,
    ),
  };
}
...
it("restores museum floor height without losing saved progress", () => {
  const world = at(MUSEUM.spawn);
  const repaired = repairWorldPositions(world);
  expect(player(repaired).position).toEqual(MUSEUM.spawn);
  expect(player(repaired).elevation).toBe(1.2);
  expect(player(repaired).money).toBe(player(world).money);
  expect(repairWorldPositions(repaired)).toBe(repaired);
});
